...
export function addRecommendedTags(currentTags, product, now = new Date()) {
...
export function decayRecommendedTags(currentTags, now = new Date()) {
    const result = {};
...
export function recommendationScore(product, currentTags) {
    return product.tags.reduce((score, tag) => score + (currentTags[tag]?.score ?? 0), 0);
}
export function buildProductFeed(products, reviews, session) {
